...
(function () {
    ...
    async function resolveShowRecordBySlug(slug, fallback) {
        ...
        return {
            ...
            title: titleMatch ? decodeInlineJsString(titleMatch[1]).trim() : (fallback && fallback.title ? fallback.title : slug),
            year: yearMatch ? decodeInlineJsString(yearMatch[1]).trim() : (fallback && fallback.year ? fallback.year : ''),
            poster: posterMatch ? decodeInlineJsString(posterMatch[1]).trim() : (fallback && fallback.poster ? fallback.poster : '')
        };
    }
    ...
    function ensureUiStyle() {
        ...
        style.textContent = `
            ...
            .${SCRIPT_ID}-watch-item-title:hover {
    ...
    function buildWatchlistItemMarkup(entry) {
        ...
        return `
            <article class="${SCRIPT_ID}-watch-item" data-state="${state}">
                <div class="${SCRIPT_ID}-watch-item-head">
                    <div>
                ...
                <div class="${SCRIPT_ID}-watch-actions">
                    <a class="${SCRIPT_ID}-link-button" href="${escapeHtml(openHref)}">Open</a>
